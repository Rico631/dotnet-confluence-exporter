...
public sealed class ValidationCollector(Compilation compilation)
{
    ...
    {
        var validators = GetAllTypes(compilation.Assembly.GlobalNamespace)
            .Where(x => IsValidatorFor(x, requestType));

        return [.. validators.SelectMany(CollectValidator)];
    }
