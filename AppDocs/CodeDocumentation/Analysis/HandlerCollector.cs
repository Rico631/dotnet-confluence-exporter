...
public sealed class HandlerCollector
{
    private readonly string RequestHandlerName;
    private readonly Compilation _compilation;
    private readonly ValidationCollector _validationCollector;
    private readonly EntityCollector _entityCollector;
    private readonly DomainEventCollector _domainEventCollector;
    private readonly CallGraphBuilder _callGraphBuilder;
    private readonly CommitDocumentationCollector _commitCollector;

    public HandlerCollector(Compilation compilation, string requestHandlerName = "IRequestHandler")
    {
        _compilation = compilation;
        RequestHandlerName = requestHandlerName;
        _validationCollector = new ValidationCollector(compilation);
        _entityCollector = new EntityCollector(compilation);
        _domainEventCollector = new DomainEventCollector(compilation);
        _callGraphBuilder = new CallGraphBuilder(compilation);
        _commitCollector = new CommitDocumentationCollector();

    }

    public IReadOnlyList<HandlerDocumentation> Collect()
    {
        return GetAllTypes(_compilation.Assembly.GlobalNamespace)
